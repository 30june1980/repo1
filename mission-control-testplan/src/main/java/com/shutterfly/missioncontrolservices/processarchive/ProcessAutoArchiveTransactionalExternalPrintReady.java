package com.shutterfly.missioncontrolservices.processarchive;
...
public class ProcessAutoArchiveTransactionalExternalPrintReady extends ConfigLoader {
    ...
    private void getResponseForPostRecieved() throws IOException {
        ...
        Response response = RestAssured.given().header("saml", config.getProperty("SamlValue")).log()
                .all()
                .contentType("application/xml").body(Util.replaceExactMatch(this.buildPayloadForPost(),"Generated","Received")).when().post(config.getProperty("BaseUrl") + config.getProperty("UrlExtensionPostFulfillment"));
        assertEquals(response.getStatusCode(), 200, "Assertion for Response code!");
        response.then().body(
                "acknowledgeMsg.acknowledge.validationResults.transactionLevelAck.transaction.transactionStatus",
                equalTo("Accepted"));
    ...
    private void getResponseForPostFulfilled() throws IOException {
        ...
        Response response = RestAssured.given().header("saml", config.getProperty("SamlValue")).log()
                .all()
                .contentType("application/xml").body(Util.replaceExactMatch(this.buildPayloadForPost(),"Generated","Fulfilled")).when().post(config.getProperty("BaseUrl") + config.getProperty("UrlExtensionPostFulfillment"));
        assertEquals(response.getStatusCode(), 200, "Assertion for Response code!");
        response.then().body(
                "acknowledgeMsg.acknowledge.validationResults.transactionLevelAck.transaction.transactionStatus",
                equalTo("Accepted"));
